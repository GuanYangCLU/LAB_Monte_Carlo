...
{
  innerRadiusOfTheTank = 0.*cm;
  outerRadiusOfTheTank = 1.6*cm;
  hightOfTheTank = 0.632*m;
  startAngleOfTheTank = 0.*deg;
  spanningAngleOfTheTank = 360.*deg;
  //bubble_x  = bubble_y  = bubble_z  =  1.6*cm;
  expHall_x = expHall_y = 2.0*cm;
  expHall_z = 0.8*m;
...
//	      
  ...
  Sci->SetMaterialPropertiesTable(myMPT1);            //Water before

  // Set the Birks Constant for the Water scintillator

  Sci->GetIonisation()->SetBirksConstant(0.126*mm/MeV);     //Water before
...
//	
  ...
  G4Tubs* waterTank_tube = new G4Tubs("Tank",innerRadiusOfTheTank,
                                            outerRadiusOfTheTank,
                                                  hightOfTheTank,
                                             startAngleOfTheTank,
                                          spanningAngleOfTheTank);

  G4LogicalVolume* waterTank_log
    = new G4LogicalVolume(waterTank_tube,Sci,"Tank");

  G4VPhysicalVolume* waterTank_phys
    = new G4PVPlacement(0,G4ThreeVector(0,0,0),waterTank_log,"Tank",
                        expHall_log,false,0);
...
//G4VPhysicalVolume* bubbleAir_phys =
      new G4PVPlacement(0,G4ThreeVector(0,0,-0.616*m),bubbleAir_log,"Bubble",
...
//
  ...
  //OpticalAirSurface
  G4double Reflectivity[num] = {1.0, 1.0};//0.3 0.5
...
// sensetive detector
  G4String SDname;
  G4VSensitiveDetector* sd_bubble = new ExN06SDBubble("/Bubble");
  G4SDManager* SDMan = G4SDManager::GetSDMpointer();
  SDMan -> AddNewDetector(sd_bubble);
  bubbleAir_log -> SetSensitiveDetector(sd_bubble);  
//  waterTank_log  -> SetSensitiveDetector(sd_bubble);
