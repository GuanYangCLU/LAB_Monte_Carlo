...
ExN06PrimaryGeneratorAction::ExN06PrimaryGeneratorAction()
{
  ...
  G4ParticleDefinition* particle = particleTable->FindParticle("opticalphoton");//e+

  particleGun->SetParticleDefinition(particle);
  particleGun->SetParticleTime(0.0*ns);
  particleGun->SetParticlePosition(G4ThreeVector(0.0*cm,0.0*cm,40.0*cm));
//srand((unsigned) time (NULL));//useless
//particleGun->SetParticleMomentumDirection(G4ThreeVector(0.,G4UniformRand(),-5.116));//(rand()%10)/9.0
  particleGun->SetParticleMomentumDirection(G4ThreeVector(0.,0.,-1.));
  particleGun->SetParticleEnergy(2.884*eV);
}
